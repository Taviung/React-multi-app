...
export default function EditTask() {
    ...
    useEffect(() => {
        if (found) {
            setTaskTitle(found.task || "");
            setEndDate(found.endDate || "");
        }
    }, [found]);

    const shouldBlock = useCallback(
        () => taskTitle.trim() !== (found?.task || "").trim() || endDate.trim() !== (found?.endDate || "").trim(),
        [taskTitle, endDate, found]
    );
    ...
    const editTask = () => {
        if (!taskTitle.trim() || !endDate.trim()) {
            alert("Please fill out both the task name and end date.");
            return;
        }

        const updated = activity.map(item =>
            String(item.id) === String(id) ? { ...item, task: taskTitle, endDate } : item
        );
        setActivity(updated);
    };
